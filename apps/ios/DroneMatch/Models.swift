...
struct Tournament: Codable, Identifiable, Hashable {
    ...
    static func formatDate(_ value: String) -> String {
        ...
        return date.formatted(.dateTime.month(.twoDigits).day(.twoDigits).hour().minute().locale(Locale(identifier: "zh_CN")))
    }
}
...
struct Registration: Codable, Identifiable {
    let id, tournamentId, teamId, teamName, tournamentTitle, status, reviewNote, createdAt, city, category: String
    let roster: [String]
    let version: Int
    var statusLabel: String { ["pending":"待审核", "approved":"已通过", "rejected":"未通过"][status] ?? status }
}
struct SessionResponse: Codable { let token: String; let account: Account }
struct SubmitRegistration: Encodable { let teamId: String; let acceptRules = true }
struct CreateTeam: Encodable { let name, city, category: String; let roster: [String]; let adultOnly = true }
