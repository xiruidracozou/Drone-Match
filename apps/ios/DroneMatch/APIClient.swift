...
struct APIClient {
  ...
  func request<T: Decodable>(
    _ path: String, token: String? = nil, method: String = "GET", body: Data? = nil
  ) async throws -> T {
    let parts = path.split(separator: "?", maxSplits: 1, omittingEmptySubsequences: false)
    var components = URLComponents(
      url: baseURL.appendingPathComponent(String(parts[0])), resolvingAgainstBaseURL: false)!
    ...
    guard let http = response as? HTTPURLResponse else {
      throw APIError(status: 0, message: "服务器响应异常")
    }
...
enum SessionStorage {
  private static let query: [String: Any] = [
    ...
    kSecAttrService as String: "com.dronematch.local", kSecAttrAccount as String: "session",
  ]
  ...
  static func save(_ token: String) throws {
    ...
    guard SecItemAdd(request as CFDictionary, nil) == errSecSuccess else {
      throw APIError(status: 0, message: "无法安全保存登录状态")
    }
  }
