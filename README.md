...
> iOS 已按 mobile-app-ui-design 完成一轮全端修改与功能修复；模拟器视觉/点击验收仍待完成。实施范围、测试和未完成项见 [移动端修复执行记录](docs/design/移动端修复执行记录.md)。
...
前轮23项API测试通过；本轮原生构建、模型回归和五栏主要页面的模拟器复核完成，修复城市名称、跨月日期、深色按钮和发布表单校验。完整社区双账号操作及多尺寸验收仍未完成，详见 [移动端修复执行记录](docs/design/移动端修复执行记录.md)。
