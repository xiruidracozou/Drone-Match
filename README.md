...
> iOS 已按 mobile-app-ui-design 完成一轮界面与功能修复，并完成五栏、双账号招募留言、外部回放及小屏最大字号的指定路径验收。实施范围、测试和未完成项见 [移动端修复执行记录](docs/design/移动端修复执行记录.md)。
...
- SwiftUI iOS：主页、赛事、生涯、视频、我的五栏；赛事筛选/详情/公开参赛队伍/对阵与比分、演示登录、队伍管理与入队、报名、社区招募/找队/约赛/志愿活动、申请与留言、个人资料、反馈和外观设置。
...
- 生涯展示实际保存的队伍、报名与社区记录，不将报名视为出场战绩；视频目前为 FAI 官方外部回放合集、搜索和本机收藏，尚无平台直播或上传服务。
...
bash scripts/ios-model-check.sh  # iOS 报名状态模型回归
bash scripts/ios-api-smoke.sh    # 实际 Swift APIClient 只读检查（需本地 API）
...
已实现主办方手动发布/调整/取消对阵、录入比赛总比分，检查同赛事内队伍和同名场地的时间冲突，并以版本号防止覆盖他人更新。生涯页依据当前队伍关系读取比赛，不能据此认定个人出场；尚无自动排赛、局制规则、积分榜或跨赛事场地模型。

前轮23项API测试通过；最新原生构建、Swift模型回归及实际客户端smoke通过。已完成五栏主要页面、双账号招募/接受/留言/退出、官方外部回放，以及SE最大辅助字号的指定路径复核；修复申请长度边界和大字号入口、筛选、日期布局。正式身份、自有直播等能力仍未接入，具体证据与边界详见 [移动端修复执行记录](docs/design/移动端修复执行记录.md)。

正式短信登录、机构入驻审核、未成年人/监护授权、真实飞手身份模型、人员跨队资格约束、名单锁定、检录、正式赛制与个人出场统计、平台视频源、约赛冲突检查、志愿者签到、内容治理、用户注销和生产部署仍未实现。当前队伍名单是成人演示字符串快照，不是完整人员身份系统；没有声称验证真实年龄或资质。
