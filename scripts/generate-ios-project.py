...
add('assets', 'isa = PBXFileReference; lastKnownFileType = image.jpeg; path = SoccerHero.jpg; sourceTree = "<group>";')
add('assetsBuild', f'isa = PBXBuildFile; fileRef = {uid("assets")};')
add('resources', f'isa = PBXResourcesBuildPhase; buildActionMask = 2147483647; files = ({uid("assetsBuild")}); runOnlyForDeploymentPostprocessing = 0;')
...
add('sourceGroup', 'isa = PBXGroup; children = (' + ','.join([uid('file:'+f.name) for f in files] + [uid('assets')]) + '); path = DroneMatch; sourceTree = "<group>";')
